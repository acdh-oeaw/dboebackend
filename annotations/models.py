...
@receiver(
    post_save, sender=Annotation, dispatch_uid="create_perms_annotation_created_by"
)
        ...
        if instance.collection.public is False:
            for curator in instance.collection.curator.all():
                ...
                if curator is not instance.collection.created_by:
                    assign_perm(
                        "delete_annotation", instance.collection.created_by, instance
                    )
                    assign_perm(
                        "change_annotation", instance.collection.created_by, instance
                    )
                    assign_perm(
                        "view_annotation", instance.collection.created_by, instance
                    )
        else:
...
@receiver(
    m2m_changed, sender=Collection.curator.through, dispatch_uid="create_perms_curator"
)
